...
- **Automatic reroute the moment presence is confirmed** — once a node's presence is confirmed via the live minimap scan, the close route recomputes fresh from the player's current (now closer) position, for the most direct final stretch.
...
- **Opportunistic switch to a resource Sku itself just detected** — piggybacks on Sku's own passive "notify on resources" minimap scanner (the same one behind its spoken proximity alerts while riding around): whenever it resolves a resource by name, this addon checks whether a remaining route node of that name is meaningfully closer (20+ yards) than the node currently being approached, and — respecting a short cooldown so it can't flap between two similar-distance nodes — switches the close route to it on the spot. The abandoned node isn't lost, just deferred; it's picked up again later exactly like any other remaining node once it's genuinely the closest one left.
