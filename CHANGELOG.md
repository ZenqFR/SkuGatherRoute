...
- The fix above uses the correct/working scan mechanism, but presence detection is still unreliable in testing even so. Leading theory: Sku's own working fallback scan only re-centers the mouse cursor on the minimap once, the very first time it ever runs in a session, then assumes it stayed there — ordinary mouse movement during play likely breaks that assumption silently. Temporary diagnostic logging (raw scan result on every miss) added to confirm before deciding on a fix.
